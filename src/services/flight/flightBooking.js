...
// Function to perform Flight Booking
export const performFlightBooking = async (dispatch, flightOffersData) => {
  const accessToken = await getAccessToken();

  const apiEndpoint = `${amadeusApiUrl}/v1/booking/flight-orders`;
  //   const payload = {
  //     data: {
  //       type: "flight-order",
  //       flightOffers: [
  ...
  try {
    const successFlightBookingResult = await post(
      apiEndpoint,
      flightOffersData
    );
    return successFlightBookingResult;
  } catch (err) {
